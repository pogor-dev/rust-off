...
pub(crate) fn generate(check: bool) {
    let grammar = fs::read_to_string(project_root().join("crates/syntax/pdf.ungram")).unwrap().parse().unwrap();
    ...
    ensure_file_contents(crate::flags::CodegenType::Grammar, syntax_kinds_file.as_path(), &syntax_kinds, check);

    let ast_tokens = generate_tokens(&ast);
    let ast_tokens_file = project_root().join("crates/syntax/src/ast/generated/tokens.rs");
    ensure_file_contents(crate::flags::CodegenType::Grammar, ast_tokens_file.as_path(), &ast_tokens, check);

    let ast_nodes = generate_nodes(kinds_src, &ast);
    let ast_nodes_file = project_root().join("crates/syntax/src/ast/generated/nodes.rs");
    ensure_file_contents(crate::flags::CodegenType::Grammar, ast_nodes_file.as_path(), &ast_nodes, check);
}
...
fn generate_nodes(kinds: KindsSrc, grammar: &AstSrc) -> String {
    ...
    let (enum_defs, enum_boilerplate_impls): (Vec<_>, Vec<_>) = grammar
        ...
        .iter()
        .map(|en| {
            let variants: Vec<_> = en.variants.iter().map(|var| format_ident!("{}", var)).sorted().collect();
    ...
    let mut docs = grammar.nodes.iter().map(|it| &it.doc).chain(grammar.enums.iter().map(|it| &it.doc));
...
fn generate_syntax_kinds(grammar: KindsSrc) -> String {
    ...
    let contextual_keywords_str_match_arm =
        grammar
            .contextual_keywords
            .iter()
            .map(|kw| match grammar.edition_dependent_keywords.iter().find(|(ed_kw, _)| ed_kw == kw) {
...
fn lower_separated_list(acc: &mut Vec<Field>, grammar: &Grammar, label: Option<&String>, rule: &Rule) -> bool {
    ...
    let (nt, repeat, trailing_sep) = match rule.as_slice() {
        [Rule::Node(node), Rule::Rep(repeat), Rule::Opt(trailing_sep)] => (Either::Left(node), repeat, Some(trailing_sep)),
        [Rule::Node(node), Rule::Rep(repeat)] => (Either::Left(node), repeat, None),
        [Rule::Token(token), Rule::Rep(repeat), Rule::Opt(trailing_sep)] => (Either::Right(token), repeat, Some(trailing_sep)),
