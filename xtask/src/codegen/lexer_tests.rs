...
pub(crate) fn generate(check: bool) {
    let crate_root_path = project_root().join("crates/lexer");
    let test_data_path = crate_root_path.join("test_data");
    ...
    let ok_tests = tests_ok.values().sorted_by(|a, b| a.name.cmp(&b.name)).map(|test| {
        ...
        let test_file = format!("test_data/ok/{test_name}.pdf");
    ...
    let err_tests = tests_err.values().sorted_by(|a, b| a.name.cmp(&b.name)).map(|test| {
        let test_name = quote::format_ident!("{}", test.name);
        let test_file = format!("test_data/err/{test_name}.pdf");
