...
impl Connection {
    ...
    /// # }
    /// ```
    pub fn initialize_start_while<C>(&self, running: C) -> Result<(RequestId, serde_json::Value), ProtocolError>
    where
        C: Fn() -> bool,
    {
        while running() {
            ...
            match msg {
                ...
                Message::Request(req) => {
                    let resp = Response::new_err(req.id.clone(), ErrorCode::ServerNotInitialized as i32, format!("expected initialize request, got {req:?}"));
                ...
                msg => {
                    return Err(ProtocolError::new(format!("expected initialize request, got {msg:?}")));
                }
            };
        }

        Err(ProtocolError::new(String::from("Initialization has been aborted during initialization")))
    }
    ...
    pub fn initialize_finish_while<C>(&self, initialize_id: RequestId, initialize_result: serde_json::Value, running: C) -> Result<(), ProtocolError>
    where
        C: Fn() -> bool,
    {
        let resp = Response::new_ok(initialize_id, initialize_result);
        ...
        while running() {
            ...
            match msg {
                ...
                msg => {
                    return Err(ProtocolError::new(format!(r#"expected initialized notification, got: {msg:?}"#)));
                }
            }
        }
    ...
    /// }
    /// ```
    pub fn initialize(&self, server_capabilities: serde_json::Value) -> Result<serde_json::Value, ProtocolError> {
    ...
    /// # }
    /// ```
    pub fn initialize_while<C>(&self, server_capabilities: serde_json::Value, running: C) -> Result<serde_json::Value, ProtocolError>
    where
        C: Fn() -> bool,
    {
    ...
    pub fn handle_shutdown(&self, req: &Request) -> Result<bool, ProtocolError> {
        ...
        match &self.receiver.recv_timeout(std::time::Duration::from_secs(30)) {
            ...
            Ok(msg) => return Err(ProtocolError::new(format!("unexpected message during shutdown: {msg:?}"))),
            Err(RecvTimeoutError::Timeout) => return Err(ProtocolError::new("timed out waiting for exit notification".to_owned())),
            Err(RecvTimeoutError::Disconnected) => return Err(ProtocolError::new("channel disconnected waiting for exit notification".to_owned())),
        }
        Ok(true)
    }
}
...
mod tests {
    ...
    fn exit_notification() {
        ...
        initialize_start_test(TestCase {
            ...
            expected_resp: Err(ProtocolError::new(format!("expected initialize request, got {notification_msg:?}"))),
        });
    }
}
