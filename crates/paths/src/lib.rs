...
impl AbsPathBuf {
    ...
    ///
    /// Panics if `path` is not absolute.
    pub fn assert_(path: PathBuf) -> AbsPathBuf {
    ...
    ///   to `.` and `..` are removed.
    pub fn push<P: AsRef<Path>>(&mut self, suffix: P) {
        self.0.push(suffix)
    }
...
impl<P: AsRef<Path> + ?Sized> PartialEq<P> for AbsPath {
    fn eq(&self, other: &P) -> bool {
        self.0.as_ref() as &Path == other.as_ref()
    }
}
...
impl AbsPath {
    ...
    pub fn join(&self, path: impl AsRef<Path>) -> AbsPathBuf {
        Path::join(self.as_ref(), path).try_into().unwrap()
    }
    ...
    pub fn file_stem(&self) -> Option<&str> {
        self.0.file_stem()?.to_str()
    }
    pub fn as_os_str(&self) -> &OsStr {
        self.0.as_os_str()
    }
    pub fn as_str(&self) -> &str {
        self.0.to_str().expect("Path contains invalid UTF-8")
    }
