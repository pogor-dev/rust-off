...
impl Symbol {
    pub fn intern(s: &str) -> Self {
        ...
        let bucket = match shard.find_or_find_insert_slot(hash, |(other, _)| other.as_str() == s, |(x, _)| Self::hash(storage, x.as_str())) {
            Ok(bucket) => bucket,
    ...
    #[inline]
    fn select_shard(storage: &'static Map, s: &str) -> (dashmap::RwLockWriteGuard<'static, RawTable<(Symbol, SharedValue<()>)>>, u64) {
...
mod tests {
    ...
    fn smoke_test() {
        ...
        drop(default);
        assert_eq!("hello world!", format!("{} {}{}", hello.as_str(), world.as_str(), bang.as_str()));
        drop(many_worlds);
        drop(more_worlds);
        drop(hello);
        drop(world);
