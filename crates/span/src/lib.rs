...
const _: () = assert!(EditionedFileId::RESERVED_HIGH_BITS + EditionedFileId::EDITION_BITS + EditionedFileId::FILE_ID_BITS == u32::BITS);
...
impl fmt::Debug for HirFileIdRepr {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MacroFile(arg0) => f.debug_tuple("MacroFile").field(&arg0.macro_call_id.0).finish(),
        }
    }
}
...
impl HirFileId {
    ...
    pub fn macro_file(self) -> Option<MacroFileId> {
        match self.0.as_u32() & Self::MACRO_FILE_TAG_MASK {
            0 => None,
            _ => Some(MacroFileId {
                macro_call_id: MacroCallId(salsa::Id::from_u32(self.0.as_u32() ^ Self::MACRO_FILE_TAG_MASK)),
            }),
        }
    }
