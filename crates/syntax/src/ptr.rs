...
use crate::{syntax_node::PdfLanguage, AstNode, SyntaxNode};

/// A "pointer" to a [`SyntaxNode`], via location in the source code.
pub type SyntaxNodePtr = rowan::ast::SyntaxNodePtr<PdfLanguage>;
...
impl<N: AstNode> AstPtr<N> {
    ...
    pub fn kind(&self) -> pdfc_parser::SyntaxKind {
        self.raw.kind()
    }
...
fn test_local_syntax_ptr() {
    use crate::{ast, AstNode, PdfDocument};

    let file = PdfDocument::parse(b"5 0 obj\n123\nendobj", pdfc_parser::Edition::CURRENT).ok().unwrap();
    let field = file.syntax().descendants().find_map(ast::IndirectObjectId::cast).unwrap();
